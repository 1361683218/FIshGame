...
const calculateObstacleHeight = () => {
  ...
  switch (difficulty.value) {
    case 'Easy':
      heightIndex = {
        up: { index1: 2, index2: 3.5},    // 增上方系数
        down: { index1: 1.5, index2: 2 }  // 增加下方系数
      }
      break
    case 'Medium':
      heightIndex = {
        up: { index1: 3, index2: 4 },
        down: { index1: 2, index2:3 }
      }
      break
    case 'Hard':
      heightIndex = {
        up: { index1: 4, index2: 4.5 },
        down: { index1: 2.5, index2: 3.5 }
      }
      break
  }

  if (obstacleToggle === 'top') {
    // 上方障碍物：使用 index1 和 index2 分别计算最小和最大高度
    const minHeight = Math.max(30, 
      baseHeight - heightIndex.up.index1 * BASE_OBSTACLE_HEIGHT)
    const maxHeight = Math.min(
      screenHeight - 50,
      baseHeight + BASE_OBSTACLE_HEIGHT * heightIndex.up.index2
    )
    
    if (minHeight >= maxHeight) {
      return maxHeight - screenHeight * 0.1
    }
    ...
    const minHeight = Math.max(
      screenHeight * 0.2,  // 最小为屏幕高度的20%
      GAME_CONFIG.BOTTOM_OBSTACLE_MIN_HEIGHT
    )
    const maxHeight = Math.min(
      screenHeight - baseHeight - 30,
      (screenHeight - baseHeight) * heightIndex.down.index2  // 使用更大的比例
    )
